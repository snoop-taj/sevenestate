...
//        }
        ...
        foreach($res_array_all as $key=>$estate_arr)
        {
            ...
            $estate_obj->adr = $estate_arr['address'];
	...
	public function index ()
	{
        ...
        foreach($options_name as $key=>$row)
        {
            ...
            if(count(explode(',', $row->values)) > 0)
            {
                $options_sel = '<option value="">'.$row->option.'</option>';
                $options_li = '';
                foreach(explode(',', $row->values) as $key2 => $val)
                {
                    $options_sel.='<option value="'.$val.'">'.$val.'</option>';
                    $this->data['options_values_arr_'.$row->option_id][] = $val;
                ...
                $this->data['options_values_'.$row->option_id] = $options_sel;
                $this->data['options_values_li_'.$row->option_id] = $options_li;
            }
        }
        ...
        //if(!empty($lang_purpose) && config_item('all_results_default') !== TRUE)
        ...
        $this->db->distinct();
        $this->db->select('property.id, property.gps, property.is_featured, property.address, property.counter_views, property.date, property.repository_id, property_user.user_id as agent_id, user.repository_id as agent_rep_id');
        $this->db->from('property');
        $this->db->where('property.is_activated', 1);
        $this->db->join('property_user', 'property.id = property_user.property_id', 'left');
        $this->db->join('user', 'property_user.user_id = user.id', 'left');
        ...
        $all_agents = $this->user_m->get_counted(array('type'=>'AGENT'), FALSE, NULL, 'properties_count DESC, user_id', '', $search_agent);
        ...
        foreach($all_agents as $key=>$agent_obj)
        {
            ...
            // [agent second image]
            if(isset($this->data['images_'.$agent_obj->repository_id][1]))
            {
                $agent['image_sec_url'] = $this->data['images_'.$agent_obj->repository_id][1]->thumbnail_url;
            }
            // [/agent second image]
        ...
        $paginated_agents = $this->user_m->get_counted(array('type'=>'AGENT'), FALSE, $agent_per_page, 'properties_count DESC, user_id', $offset, $search_agent);
